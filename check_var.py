...
def calib_noise_model(frms, h, w, rects):
    ...
    for i, frm in enumerate(frms):
        ...
        mean_pre = mean
        mean = (1 - 1 / (i + 1)) * mean + 1 / (i + 1) * raw
        var = var + (raw - mean_pre) * (raw - mean)
...
def check_noise_model(frms, h, w, rects, K, B):
    ### memory limited, recursively calc mean, var
    mean = 0
    var = 0

    frm = np.random.choice(frms)
    raw = np.fromfile(frm, np.uint16).reshape(int(h), int(w)).astype(np.float32)
    raw_sub = subpixel(raw)
    ...
    for rect in rects:
        ...
        crop_sub = raw_sub[lt_y: rb_y, lt_x: rb_x, :]
        mean = np.mean(crop_sub[:, :, 0])
        var = np.var(crop_sub[:, :, 0])
...
if __name__ == "__main__":
    ...
    # K = 1.3703383
    # B = -85.85382
    K = [1.3046978, 1.3727844, 1.3705341, 1.3704913]
    B = [-76.5213, -86.26315, -85.88168, -85.87596]

    check_noise_model(raw_files1, h, w, rects, K, B)
